...
describe('Dashboard Data Formatting', () => {
  describe('aggregateByDay', () => {
    it('should aggregate click events by date', () => {
      const clickEvents = [
        makeClickEvent({ clicked_at: new Date('2026-04-01T08:00:00Z').toISOString(), source: 'social', device: 'mobile' }),
        makeClickEvent({ clicked_at: new Date('2026-04-01T14:00:00Z').toISOString(), source: 'direct', device: 'desktop' }),
        makeClickEvent({ clicked_at: new Date('2026-04-02T08:00:00Z').toISOString(), source: 'social', device: 'mobile' }),
      ];
    ...
    it('should format dates consistently (YYYY-MM-DD)', () => {
      const clickEvents = [
        makeClickEvent({ clicked_at: new Date('2026-04-13T23:59:59Z').toISOString() }),
      ];
  ...
  describe('aggregateByWeek', () => {
    it('should aggregate click events by week', () => {
      const clickEvents = [
        makeClickEvent({ clicked_at: new Date('2026-04-01T08:00:00Z').toISOString(), source: 'social', device: 'mobile' }),
        makeClickEvent({ clicked_at: new Date('2026-04-06T08:00:00Z').toISOString(), source: 'direct', device: 'desktop' }),
        makeClickEvent({ clicked_at: new Date('2026-04-07T08:00:00Z').toISOString(), source: 'social', device: 'mobile' }),
      ];
    ...
    it('should return week numbers and start dates', () => {
      const clickEvents = [
        makeClickEvent({ clicked_at: new Date('2026-04-01T08:00:00Z').toISOString() }),
      ];
  ...
  describe('formatDashboardData', () => {
    it('should format complete dashboard response', () => {
      const dashboardData = {
        shortLinks: [{ code: 'abc123', destination_url: 'https://example.com', click_count: 10 }],
        clickEvents: [
          makeClickEvent({ source: 'social', device: 'mobile' }),
        ],
    ...
    it('should sort top links by click count descending', () => {
      const dashboardData = {
        shortLinks: [
          { code: 'link1', destination_url: 'https://one.com', click_count: 50 },
          { code: 'link2', destination_url: 'https://two.com', click_count: 100 },
          { code: 'link3', destination_url: 'https://three.com', click_count: 25 },
        ],
        clickEvents: [],
        dateRange: { startDate: '2026-04-01', endDate: '2026-04-13' },
      };
