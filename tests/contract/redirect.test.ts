...
describe("GET /api/redirect/[code]", () => {
  it("returns 302 for existing code", async () => {
    ...
    const createRequest = new Request("http://localhost:3000/api/shorten", {
      ...
      body: JSON.stringify({ destination_url: "https://example.com/redirected" })
    });

    const created = await POST(createRequest);
    const payload = await created.json();

    const response = await GET(new Request("http://localhost:3000/api/redirect/" + payload.code), {
  ...
  it("returns 404 when code does not exist", async () => {
    ...
    const response = await GET(new Request("http://localhost:3000/api/redirect/missing"), {
      params: { code: "missing" }
    });
